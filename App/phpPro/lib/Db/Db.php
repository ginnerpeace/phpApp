...
class Db{
    /* protected $config = array(
        ...
        'dbname'=>'test,test',
        'port'=>'3306,3306',
        'username'=>'masteruser,slaveuser',
        'password'=>'masteruser123,slaveuser123',
    ...
    protected function query($sql,$getsql = false){
        ...
        if(!empty($this->bind)){
			...
			$sql = strtr($this->sql, array_map("parseVal", $this->bind));
        }
    ...
    private function connect($config='',$identify = 0,$reconnect = false){
        if(!isset($this->_links[$identify])){
             ...
             try{
				 ...
				 $this->_links[$identify] = new PDO($config['dsn'],$config['username'],$config['password']);
				 //连接成功以后设置字符集编码为 utf8;
				 $this->_links[$identify]->query("set names utf8");
